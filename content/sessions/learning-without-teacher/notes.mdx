<BigIdea>
Until now, every model had labels. Most data in the world has no labels. Unsupervised learning finds patterns in unlabeled data. We'll derive the k-means clustering algorithm, code it from scratch, and meet word embeddings — the bridge to language models in Session 15.
</BigIdea>
...
Open Spotify. Show your "Discover Weekly." Pick a song you've never heard. Play 5 seconds. Stop.

*"This song was put on my playlist by Spotify's recommendation system. Spotify has never been told 'I like this song.' I've never rated it. So how did it know? Today: how machines find patterns when nobody tells them what to look for."*
...
The most famous clustering algorithm. We'll derive it from a clear objective.

**Setup.** A dataset of n points in some feature space. We believe there are K natural groups. We want to find them.

**Define what "good clustering" means.** For each point, it belongs to one cluster. Each cluster has a "center" (centroid) <Math tex="\mu_k" />. We want each point to be close to its cluster's center.
...
**Better way: vectors.** Assign each word a *vector* of numbers (say 300 numbers). The vector is the word's **embedding**. We pick embeddings such that *similar words have similar vectors*. "Cat" and "dog" close. "Cat" and "Tuesday" far.

Who decides similarity? Here's the unsupervised part: we let the model figure it out from context. *"You shall know a word by the company it keeps"* (Firth, 1957). Words used in similar contexts (cat and dog both appear near "pet," "vet," "leash") get similar vectors.
...
Some bias examples (reminder of Session 3):
...
300-dimensional vectors are hard to visualize. We can squish them to 2D using PCA (principal component analysis, a classic dimensionality-reduction algorithm).
...
<Vocab>
  ...
  <Term name="K-means">The iterative clustering algorithm we just derived.</Term>
...
**Part 1: K-Means Lab.** Drop dots on a 2D plot. Set K. Run k-means. Watch centroids move and points get re-assigned each iteration.
...
**Part 2: Word Arithmetic Explorer.** Type three words. The model returns the closest match.
...
Yes, but now they're learned *inside* large language models, not as a separate step. Modern transformers (Session 15) build context-dependent embeddings on the fly.
  </Q>
...
<Bridge to="Session 15 — How ChatGPT works">
Word embeddings are how language models start understanding text. But what happens after the embedding step? How does ChatGPT actually generate sentences? Next time: the architecture that powers all modern AI — the transformer — and we'll derive its central mechanism, attention.
</Bridge>
